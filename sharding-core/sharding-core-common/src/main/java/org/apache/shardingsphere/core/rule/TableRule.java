...
public final class TableRule {
    ...
    public TableRule(final TableRuleConfiguration tableRuleConfig, final ShardingDataSourceNames shardingDataSourceNames, final String defaultGenerateKeyColumn) {
        ...
        /**
         * 通过SPI service loader把所有分布式主键生成策略加载到
         * {@link NewInstanceServiceLoader#SERVICE_MAP} 的Map中，SPI加载方式通过
         * {@link ServiceLoader#load(java.lang.Class)} 到 {@value "META-INF/services/"}当中加载指定配置类
         * 所有的分布式主键生成类都继承接口{@link org.apache.shardingsphere.spi.keygen.ShardingKeyGenerator}
         * 该接口当中有{@code String getType()}方法,通过该方法返回值{@code String}与我们配置的{@link KeyGeneratorConfiguration.type}
         * 主键生成类型进行匹配，找到对应主键生成策略，如snowflake对应{@link SnowflakeShardingKeyGenerator}
         * 自定义主键生成类在{@code getType}该方法当中返回唯一的标识即可,如"type = TlYgGenerator",大小写均可
         */
